...
\begin{tikzpicture}[auto, node distance=1cm,>=latex']
    \node [block, node distance=1.5cm] (Gp) {$g_p(s)$};
    \node [block, left of=Gp, node distance=3cm] (Gc) {$g_c(s)$};
...
%            node distance=3cm] (system) {System};
	\node [block, above of=sum, node distance=1.75cm] (Gd) {$g_d(s)$};
	...
	\node [block, below of=h, node distance=1.75cm] (Gm) {$g_m(s)$};
...
%    \draw [draw,->] (input) -- system {$r$} (sum);
    ...
    \draw [->, line width=0.5mm] (Gm) -| (comparator);
